...
global I0 IG I1 I2 x0_init x0_fin t0 a b h0 hG kn anm1 bnm1 ...
       kn1ga kn1gb tnm1 tn mutr leA G Gnm1 M N K0 x0 KG ...
...
for n=2:N
    ...
    if mutr >= 0
        ...
        tnao = t1ga;
        tnm1ao = tnm1*ones(1,length(t1ga));
        tnbo = tn*ones(1,length(t1gb));
        tnm1bo = t1gb;
    else
        ...
        tnao = tn*ones(1,length(t1ga));
        tnm1ao = t1ga;
        tnbo = t1gb;
        tnm1bo = tnm1*ones(1,length(t1gb));
    end
    ...
    % Mesh Plot -----------------------------------------------------------
    figure(1)
    PlotMesh(x1, x2, x1r, x1o, x1ga, x1gb, x2nm1, ...
...
%     b_vec(I0) = b_vec(I0) + b_bcx0fin;
    ...
    % Solution Plot -------------------------------------------------------
    figure(2)
    PlotSolutiondG0(U, x0, G, Gnm1, amin, amax, bmin, bmax, ...
                                    tna, tnm1a, tnb, tnm1b)
    hold on
...
%     figure(2)
%     PlotSolutiondG0(U_q2p, x0, G, Gnm1, amin, amax, bmin, bmax, ...
%                                     tna, tnm1a, tnb, tnm1b)
%     hold on
...
errexa_3pGL = ErrorL2NormSpace(U, @(x)(u_func(x,tn)), x0, G)

error_quadTrapezoidal = ErrorL2NormSpaceQuadTrapezoidal(U, Ua, x0, G, a, b)
...
% disp(' --- THE ERRORS --- ');
%
% disp('L2-norm of the error of the exact dG(0)-FE-solution');
% disp([num2str(errexa)]);
% disp(' ');
