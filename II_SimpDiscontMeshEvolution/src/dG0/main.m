...
% parameters_debug_TimeJumpVector
...
for n=2:N
    ...
    % Mesh Plot -----------------------------------------------------------
    figure(1)
    PlotMesh(G, x1, x2, x1r, x1o, x1ga, x1gb, x2nm1, ...
...
%     b_vec(I0) = b_vec(I0) + b_bcx0fin;
    ...
    figure(2)
    PlotSolutiondG0(U, x0, G, G, a, a, b, b, ...
                                    tna, tnm1a, tnb, tnm1b)
    hold on
    axis([x0_init x0_fin t0 T])
    xlabel('$x$', 'interpreter', 'latex', 'Fontsize', 24);
...
% FE-Sol err, Ua - U
% error = ErrorL2NormSpaceQuadTrapezoidal(U, Ua, x0, G, a, b);
error = ErrorL2NormSpace(U, @(x)(u_func(x,tn)), x0, G);

disp('L2-norm of the error of the dG(0)-FE-solution'); 
disp([num2str(error)]);
disp(' ');
